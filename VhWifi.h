...
const char CSS_SHARED[] PROGMEM = "<style>"
	...
	"button, div.wifiNode, #bluetooth label{"
    ...
    "}"
	"button:active, div.wifiNode:active, #bluetooth label:active{"
	...
	"#bluetooth{"
		"display:flex;"
	"}"
	"#bluetooth input{ width:auto; }"
	"#bluetooth label{ margin:0.1em !important;}"
	...
	"div.wifiNode{"
		...
		"margin:0.5em 0;"
	"}"
	"button:hover, div.wifiNode:hover, #bluetooth label:hover{"
    	"transition:none;"
        "border-color: #FFF;"
    "}"
...
const char JS_SHARED[] PROGMEM = "\n"
	...
	"if(path === '/wifi' || path === '/0wifi'){\n"
		...
		"if(sl){"
			...
			"sl.max = 10080;\n"
		"}\n"
		// Make bluetooth into radio buttons
		"const bt=GID('enable_bluetooth');\n"
		"if(bt){\n"
			"const v = +bt.value;\n"
			"const newEl = document.createElement('div');\n"
			"newEl.id = 'bluetooth';\n"
			"newEl.innerHTML=\n"
				"'<label>ON <input type=\"radio\" name=\"enable_bluetooth\" value=1 '+(v==1 ? \"checked\" : \"\")+' /></label>'+\n"
				"'<label>OFF <input type=\"radio\" name=\"enable_bluetooth\" value=0 '+(v!=1 ? \"checked\" : \"\")+' /></label>'\n"
			";\n"
			"bt.parentNode.replaceChild(newEl, bt);\n"
		"}"
