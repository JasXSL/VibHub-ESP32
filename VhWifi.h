...
const char CSS_SHARED[] PROGMEM = "<style>"
	...
	"#bluetooth label{ margin:0.5vmax !important;}"
...
const char JS_SHARED[] PROGMEM = "\n"
	...
	"if(path === '/wifi' || path === '/0wifi'){\n"
		...
		"const sl=document.getElementById('sleep_after_min');"
		"if(sl){"
			"sl.type = 'number';\n"
			"sl.min = 0;\n"
			"sl.max = 10080;\n"
			"sl.title='Turns the device off after this many minutes. Use 0 to not use auto sleep.';\n"
		"}\n"
		// Make bluetooth into radio buttons
		"const bt=document.getElementById('enable_bluetooth');\n"
		"if(bt){\n"
			"const v = +bt.value;\n"
		...
		// Event bindings
		// Reset buttons
		"const resetHost = document.getElementById('resetHost');"
		"if(resetHost){"
			"resetHost.onclick = function(){ this.parentNode.nextSibling.value='vibhub.io'; }\n"
			"resetHost.onclick = function(){ this.parentNode.nextSibling.value='80'; }\n"
		"}"
